// Routing fix for upstream MCP TS SDK issue #1186 — "Zombie Task Collision in
// StreamableHTTPServerTransport". See the longer explanation in index.ts
// `onStart` for the bug's history.
//
...
// (Copied verbatim from the sister project's proven implementation.)
 ...
 *
 *  - **passthrough**: no `requestId`, or ≤1 connection matches — the original
 *    transport behavior is correct; defer to it.
...
/**
 * Minimal shape of the upstream `StreamableHTTPServerTransport` private surface
 * we depend on. Reaching into these underscored members is fragile; see comment
 * in `index.ts` `onStart` for the upstream removal condition.
 */
...
/**
 * Direct-write replacement for the buggy `find()`-based routing in agents/mcp's
 * `transport.send`. Mirrors what the original does at the `writeSSEEvent` layer
 * but with a known-correct `connection`.
 */
 ...
 * `error`. Notifications have no id; requests have `method`.
 */
